...
export async function generate(ir: TAxiomIR, outRoot = process.cwd(), profile?: string): Promise<{ artifacts: Artifact[]; manifest: Manifest }> {
  ...
  // Initialize artifact store for content caching
  const artifactStore = new ArtifactStore(outRoot);
  ...
  async function writer(rel: string, content: string) {
    ...
    // Store artifact content in cache by SHA256
    await artifactStore.put(contentSha256, content);
  ...
  for (const agent of ir.agents) {
    for (const item of agent.emit) {
      ...
      if (!emitter) {
        ...
        await writer(placeholderPath, `# ${subtype} (placeholder)\nGenerated by AXIOM.`);
        continue;
      }
  ...
  await writer("manifest.json", JSON.stringify(manifest, null, 2));
