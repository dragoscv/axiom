  ...
  for (const agent of ir.agents) {
    ...
    for (const chk of agent.checks) {
      if (/http\./.test(chk.expect) && !caps.has("net")) {
        diags.push({ message: `Check "${chk.name}" uses http.* but capability net(...) is missing`, path: ["agents", agent.name, "checks", chk.name] });
      }
      if (/scan\.artifacts\./.test(chk.expect) && !caps.has("fs")) {
        diags.push({ message: `Check "${chk.name}" scans artifacts but capability fs(...) is missing`, path: ["agents", agent.name, "checks", chk.name] });
      }
    }
  }
