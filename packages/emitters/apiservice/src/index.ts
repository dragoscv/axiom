...
const emitter: Emitter = {
  subtype: "api-service",
  async generate(ctx) {
    const appName = ctx.agent.name || "api";
    const isEdge = ctx.profile === "edge";
    const isBudget = ctx.profile === "budget";

    // README
    ctx.write("README.md", `# ${appName.toUpperCase()} API Service
...
`);
    ...
    ctx.write("src/index.ts", `import http from 'node:http';
...
const PORT = ${isEdge ? '8787' : '4000'};
server.listen(PORT, () => {
  console.log(\`[${appName}] API listening on http://localhost:\${PORT}\`);
  console.log(\`[${appName}] OpenAPI spec: http://localhost:\${PORT}/openapi\`);
});
`);
